...
Qué hace
  ...
  4. Reescribe la galería dentro de index.html (entre GALERIA:INICIO y FIN).
...
def main():
    ...
    presentes = {p.name: p for p in IMG.iterdir() if p.suffix.lower() in EXTENSIONES}
    lista = [(n, a) for n, a in FOTOS if n in presentes]
    faltan = [n for n, _ in FOTOS if n not in presentes]
    extras = sorted(n for n in presentes if n not in {n for n, _ in FOTOS})
    if faltan:
        print("Aviso: no están en IMG/ ->", ", ".join(faltan))
    ...
    if INDEX.exists():
        ...
        if patron.search(txt):
            ...
            INDEX.write_text(txt, encoding="utf-8", newline="\n")
            print(f"index.html actualizado con {total} fotos")
        else:
            print("index.html no tiene los marcadores GALERIA:INICIO / GALERIA:FIN")
    print(
